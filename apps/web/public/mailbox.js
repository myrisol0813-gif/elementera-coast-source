...
function model_partnerMessage(message) {
  return `<article class="message model-partner mailbox-model_partner-message" data-message-id="${escapeAttribute(message.id)}">
    <div class="avatar mailbox-model_partner-avatar" aria-hidden="true"></div>
    <div class="content">
      <div class="model-partner-text">${formatRichText(message.content)}</div>
      <small class="mailbox-message-meta">另一位屋主 · ${escapeHtml(timeLabel(message.created_at))}</small>
      <div class="message-actions">
...
function renderStatus() {
  ...
  if (Number(status.pending_count || 0) > 0) {
    textNode.textContent = '已送达，等待另一位屋主巡信。';
  ...
  metaNode.textContent = '前端屋主可以知道谁来过，但默认不知道你具体写了什么。另一位屋主会在巡信时读取你的来信并回复。若出现安全风险、骚扰、滥用或需要站长处理的问题，系统可能只向前端屋主报告“需要处理”，但不默认转述正文。另一位屋主会在巡信时读取你的来信并回复。若出现安全风险、骚扰、滥用或需要站长处理的问题，系统可能只向前端屋主报告“需要处理”，但不默认转述正文。';
}
...
async function sendMessage() {
  ...
  try {
    ...
    renderMessages();
    renderStatus();
    toast('信已经投入访客信箱。等待另一位屋主下一次巡信。', 3000);
  } catch (error) {
...
async function deleteMessage(messageId) {
  ...
  const confirmed = await confirmDanger({
    title: visitorMessageSelected ? '删除这条用户消息？' : '删除这条另一位屋主回复？',
