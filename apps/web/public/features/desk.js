...
const ENTRY_TYPE_LABELS = Object.freeze({ memory: '记忆', seed: '线索' });
...
function crossWindowDeskDetails(section = {}) {
  ...
  const messageDetail = messageGroups.length
    ? `<div class="desk-message-list">${messageGroups.map((group) => `<section><b>${escapeHtml(sources.find((source) => source.conversation_id === group.conversation_id)?.title || '来源窗口')}</b>${(Array.isArray(group.messages) ? group.messages : []).map((message) => {
      const role = message.role === 'assistant' ? '另一位屋主' : '屋主';
...
function webSearchDeskDetails(section = {}) {
  ...
  return [
    ...
    section.provider_query_returned === false
      ? '<p class="desk-slip-note">Provider search tool 未向系统回传模型内部生成的原始搜索 query；这里如实显示当前用户请求，不伪造 query。</p>'
      : '',
...
function currentDeskSlip(history) {
  return currentModelPartner(history)?.desk_slip || null;
}
...
function modelEchoRow(value = {}) {
  ...
  return `<article class="desk-model-echo"><strong>模型后端返回原文</strong><span>${escapeHtml(status)}</span><p class="desk-slip-note">这是本轮模型 API 返回的推理文本、用量、模型、供应商、完成状态、工具调用和脱敏元数据。它只给前端屋主查看，不会递给模型，也不会进入记忆或线索。详细内容请回到对应回复下方查看。</p></article>`;
}
...
export function createDesk({ router, toast }) {
  ...
  function deskView() {
    ...
    const globalExcerpt = slip.global_excerpt || { label: '核心回响', status: '未设置', injection: 'empty', estimated_tokens: 0, length: 0, content: '' };
    ...
    return {
      ...
      body: `<section class="desk-slip-list">
        ...
        ${deskRow(contextBudget.label || '上下文预算', budgetStatus, [
          ...
          paragraph('核心回响注入：', contextBudget.global_excerpt || globalExcerpt.injection || 'empty'),
          contextBudget.exceeds_comfort_ceiling ? '<p class="desk-slip-note">当前组包超过 comfort ceiling；系统没有静默截断核心回响。</p>' : '',
        ].join(''))}
        ...
        ${deskRow(globalExcerpt.label || '核心回响', excerptStatus, [sourceNote(globalExcerpt.description), globalExcerpt.content ? deskText(globalExcerpt.content) : emptyNote()].join(''))}
        ...
        ${deskRow(workbench.label || '工具调用记录', workbench.status, workbenchDetail)}
