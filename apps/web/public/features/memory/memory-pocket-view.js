...
export function createPocketViews({ currentPockets }) {
  function pocketCard(pocket) {
    ...
    return `<article class="feature-card feature-prose" data-pocket-id="${escapeAttribute(pocket.id)}">
      ...
      <p class="feature-note">只有确认后才会进入记忆库或线索库。种子是可生长方向，不是已确认事实。</p>
      ${provenance}
      <div class="button-row">
        ...
        <button type="button" data-action="memory:pocket-resolve" data-id="${escapeAttribute(pocket.id)}" data-destination="seed">写入线索库</button>
