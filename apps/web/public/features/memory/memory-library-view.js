...
export function createLibraryViews({ runtime, currentId, currentPockets, chat }) {
  ...
  function libraryControls() {
    ...
    return `<div class="memory-tabs memory-tabs-five" role="tablist" aria-label="轨迹记忆入口">
      ...
      <button class="${runtime.libraryTab === 'seed' ? 'is-active' : ''}" type="button" data-action="memory:tab" data-scope="seed">线索库</button>
  ...
  function memoryView() {
    ...
    return {
      ...
      body: `${libraryControls()}<section class="feature-group"><div class="feature-card">
        <button class="feature-row" type="button" data-action="memory:pockets"><span><strong>待确认区 · ${currentPockets().length}</strong><small>只有确认后才会进入记忆库或线索库。</small></span><span>›</span></button>
      </div></section>${entryGroup(isSeed ? '线索库' : '记忆库', entries, isSeed ? '这里还没有种子。' : '这里还没有长期记忆。')}`,
    };
  }
