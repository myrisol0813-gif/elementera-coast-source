...
export function createModelsActions({
  chat,
  router,
  toast,
  getSearchDraft,
  ...
  async function selectChat(modelId, { closePicker = false } = {}) {
    ...
    toast('模型已切换', 2000);
    if (closePicker) quickPicker.closeQuickPicker();
  }
  ...
  async function handleSubmit(name) {
    ...
    setSearch(getSearchDraft().trim());
    await router.refresh({ preserveScroll: false });
  }

  function handlerFor(context, handlers) {
    const handlerName = HANDLER_NAMES[context.eventType];
    return handlerName ? { handlerName, handler: handlers[handlerName] } : null;
  }

  function ownsEvent(_event, context, handlers) {
    const entry = handlerFor(context, handlers);
    if (context.namespace !== 'models' || typeof entry?.handler !== 'function') return false;
    return { preventDefault: context.eventType === 'click' || context.eventType === 'submit' };
  }

  function handleEvent(event, context, handlers) {
    const entry = handlerFor(context, handlers);
    return entry?.handler?.(context.name, context.target, event);
  }
