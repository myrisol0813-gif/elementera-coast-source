...
function runLine(run) {
  ...
  return `<div class="furniture-run is-${ok ? 'success' : 'error'}"><p><span>${ok ? '✓' : '!'}</span>${escapeHtml(label + suffix)}</p>${items || extra ? `<ul>${items}${extra}</ul>` : ''}${error}</div>`;
}

export function renderFurnitureBubble(value, { conversationId = '' } = {}) {
  const runs = safeRuns(value);
  ...
  const actionLog = loggableIds.length
    ...
    : '';
  return `<details class="chat-furniture">
    <summary><span>本轮工具</span><strong>使用了 ${runs.length} 件工具</strong></summary>
    <div class="furniture-body">${runs.map(runLine).join('')}${actionLog}</div>
  </details>`;
}
