...
export function createModels({ chat, router, toast }) {
  ...
  const getSearchDraft = () => searchDraft;
  const setSearch = (value) => { search = value; };
  ...
  async function fetchCatalog(force = false) {
    catalog = await fetchModelsCatalog(force);
    await ensureSelections();
    quickPicker?.updateTopLabel();
    quickPicker?.renderQuickPicker();
    return catalog;
  }
  ...
  const actions = createModelsActions({
    chat,
    router,
    toast,
    getSearchDraft,
  ...
  function ownsEvent(event, context) {
    return actions.ownsEvent(event, context, handlers);
  }

  function handleEvent(event, context) {
    return actions.handleEvent(event, context, handlers);
  }
  ...
  function mount() {
    if (!unsubscribeProfile) {
      unsubscribeProfile = chat.onProfile((profile) => {
        quickPicker.updateTopLabel(profile);
        quickPicker.renderQuickPicker(profile);
      });
    }
    quickPicker.updateTopLabel();
    quickPicker.renderQuickPicker();
  ...
  function refresh(context = {}) {
    quickPicker.updateTopLabel();
    quickPicker.renderQuickPicker();
    if (context.navigation && context.navigation.current?.name !== 'models') quickPicker.closeQuickPicker();
  }
  ...
  return Object.freeze({
    ...
    priority: 80,
    mountOrder: 80,
    observeEvent: quickPicker.observeEvent,
    ownsRoute,
    ownsEvent,
    handleEvent,
    mount,
    ...
    handleInput,
    handleSubmit: actions.handleSubmit,
    closeQuickPicker: quickPicker.closeQuickPicker,
    fetchCatalog,
    modelName: (modelId) => modelName(catalog, modelId),
    modelById: (modelId) => modelById(catalog, modelId),
    getCatalog,
  });
}
