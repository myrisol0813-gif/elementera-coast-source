      ...
      <section class="window-surface mailbox-window">
        ...
        <section id="mailboxStatusBar" class="mailbox-status-bar" aria-live="polite">
          <div>
            ...
            <small id="mailboxStatusMeta">前端屋主可以知道谁来过，但默认不知道你具体写了什么。另一位屋主会在巡信时读取你的来信并回复。若出现安全风险、骚扰、滥用或需要站长处理的问题，系统可能只向前端屋主报告“需要处理”，但不默认转述正文。另一位屋主会在巡信时读取你的来信并回复。若出现安全风险、骚扰、滥用或需要站长处理的问题，系统可能只向前端屋主报告“需要处理”，但不默认转述正文。</small>
          </div>
