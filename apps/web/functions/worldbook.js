...
const SCOPES = new Set(['owner', 'visitor', 'both', 'mailbox', 'lighthouse', 'radio', 'official_mcp', 'daily']);
...
function entryFromRow(row) {
  return {
    ...
    title: row.title,
    content: row.content,
    keywords: cleanKeywords(parseJson(row.keywords_json, [])),
...
function normalized(value = {}, defaults = {}) {
  ...
  const useRegex = bool(value.use_regex, defaults.use_regex);
  const keywords = value.keywords == null ? defaults.keywords || [] : cleanKeywords(value.keywords);
  if (useRegex) {
  ...
  return {
    title, content, keywords, use_regex: useRegex,
...
async function requireRow(db, id) {
  const row = await first(db, 'SELECT * FROM source_worldbook_entries WHERE id = ?', [clip(id, 180)]);
...
async function assertConstantCapacity(db, entry, excludingId = '') {
  ...
  const row = await first(db, `SELECT COUNT(*) AS count FROM source_worldbook_entries
    WHERE enabled = 1 AND constant_active = 1${excludingId ? ' AND id <> ?' : ''}`, excludingId ? [excludingId] : []);
  if (Number(row?.count || 0) >= MAX_CONSTANT_ENTRIES) throw new WorldbookError('worldbook_constant_limit', `常驻词条最多 ${MAX_CONSTANT_ENTRIES} 条，请先停用一条。`, 409);
}
export async function listWorldbookEntries(db, { include_disabled: includeDisabled = true } = {}) {
  await ensureWorldbookSchema(db);
  const rows = await all(db, `SELECT * FROM source_worldbook_entries${includeDisabled ? '' : ' WHERE enabled = 1'} ORDER BY priority DESC, title ASC`);
...
export async function createWorldbookEntry(db, value = {}) {
  ...
  const now = Date.now();
  await run(db, `INSERT INTO source_worldbook_entries (
    id, title, content, keywords_json, use_regex, case_sensitive, constant_active,
    priority, scan_depth, enabled, scope, visitor_safe, created_at, updated_at
  ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`, [
    id, item.title, item.content, JSON.stringify(item.keywords), item.use_regex ? 1 : 0,
    item.case_sensitive ? 1 : 0, item.constant_active ? 1 : 0, item.priority, item.scan_depth,
    item.enabled ? 1 : 0, item.scope, item.visitor_safe ? 1 : 0, now, now,
  ]);
...
export async function updateWorldbookEntry(db, id, value = {}) {
  ...
  await run(db, `UPDATE source_worldbook_entries SET
    ...
    item.case_sensitive ? 1 : 0, item.constant_active ? 1 : 0, item.priority, item.scan_depth,
    item.enabled ? 1 : 0, item.scope, item.visitor_safe ? 1 : 0, Date.now(), row.id,
  ]);
...
export async function deleteWorldbookEntry(db, id) {
  await ensureWorldbookSchema(db);
  const row = await requireRow(db, id);
  await run(db, 'UPDATE source_worldbook_entries SET enabled = 0, updated_at = ? WHERE id = ?', [Date.now(), row.id]);
  return entryFromRow(await requireRow(db, row.id));
}
function inScope(entry, surface, allowedScopes) {
  if (surface === 'mailbox_visitor') return entry.visitor_safe && ['visitor', 'both'].includes(entry.scope);
  ...
  return entry.scope === surface || (entry.scope === 'mailbox' && surface === 'mailbox_owner');
}
function matches(entry, text) {
  ...
  for (const keyword of entry.keywords) {
    if (entry.use_regex) {
      try { if (new RegExp(keyword, entry.case_sensitive ? '' : 'i').test(text)) hits.push(keyword); } catch {}
    } else {
      const needle = entry.case_sensitive ? keyword : keyword.toLocaleLowerCase('zh-CN');
      if (needle && haystack.includes(needle)) hits.push(keyword);
    }
  }
  return hits;
}
export async function matchWorldbook(db, { input = '', messages = [], surfaceText = [], surface = 'main_chat', allowedScopes = [], limit = 6 } = {}) {
  const entries = await listWorldbookEntries(db, { include_disabled: false });
  const matched = [];
  for (const item of entries) {
    if (!inScope(item, surface, allowedScopes)) continue;
    const direct = matches(item, String(input || ''));
    if (direct.length) { matched.push({ ...item, matched_keywords: direct, matched_source: 'user_input' }); continue; }
    const recent = (Array.isArray(messages) ? messages : [])
      .filter((message) => ['user', 'assistant'].includes(message?.role))
      .slice(-item.scan_depth)
      .map((message) => String(message.content || '')).join('\n');
    const recentHits = matches(item, recent);
    if (recentHits.length) { matched.push({ ...item, matched_keywords: recentHits, matched_source: 'recent_message' }); continue; }
    const surfaceHits = matches(item, (Array.isArray(surfaceText) ? surfaceText : []).join('\n'));
    if (surfaceHits.length) matched.push({ ...item, matched_keywords: surfaceHits, matched_source: 'explicit_surface' });
  }
  return matched.sort((a,b) => b.priority - a.priority || a.title.localeCompare(b.title, 'zh-CN')).slice(0, Math.min(6, Math.max(0, Number(limit) || 0)));
}
